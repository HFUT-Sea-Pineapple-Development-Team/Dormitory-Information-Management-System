        ...
        <table class="table table-striped table-bordered table-hover datatable">
            ...
            <c:forEach items="${notices}" var="notice" varStatus="stat" >
                <tr>
                    <td>${stat.index}</td>
                    <td>${notice.time }</td>
                    <td>${notice.receivers_num }</td>
                    <td>${notice.title }</td>
                    <td><button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location='notice.action?action=Check&id=${notice.id}'">查看具体内容</button>&nbsp;
                    <td> <button class="btn btn-mini btn-danger" type="button" onclick="noticeDelete(${notice.id})">删除</button></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
